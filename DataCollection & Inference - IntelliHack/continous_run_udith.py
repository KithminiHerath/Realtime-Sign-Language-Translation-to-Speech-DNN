...
n_features=17
model_dir='conv_lstm_full_new_2.h5'
...
def predict(ep,gp): #csv_files
    ...
    ap = "accelerometer" + str(time_stamp)
    op = "orientationEuler" + str(time_stamp)
...
def infer(emg_file,gyro_file):
    ...
    # min-max scaler
    X_min = np.loadtxt('DNNX_min.txt', dtype=float)
    X_max = np.loadtxt('DNNX_max.txt', dtype=float)
    X = (processed.transpose()-X_min)/(X_max-X_min)


    start = time.time()
    # read trained model
    trained_model = keras.models.load_model('final_model.h5')
...
while(1):
    ...
    for file in files:
        ...
        if(file[-3:]=='csv' and file[0]=='e' and file not in dict and pair_name(file) in files):
            ...
            print(predict(file,pair_name(file)))
