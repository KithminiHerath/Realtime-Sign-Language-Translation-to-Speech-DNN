...
//Constructor for a softmax layer
//name = name of the softmax layer eg: Softmax1, Softmax2, ...
//shape = shape of the output vector
SoftmaxLayer::SoftmaxLayer(cl_device_id device_id, cl_context context, cl_command_queue command_queue, std::string name, Shape shape) :
...
//Deconstructor for a softmax layer
SoftmaxLayer::~SoftmaxLayer()
{}

//Function to do softmax activation on a layer
Matrix& SoftmaxLayer::forward(Matrix& Z) {
	...
	//Create the kernel Softmax Activation
	cl_kernel kernel = clCreateKernel(program, "softmaxActivation", &ret);
	if (ret != CL_SUCCESS) {
